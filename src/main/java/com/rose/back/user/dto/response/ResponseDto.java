...
public class ResponseDto {
    ...
    public ResponseDto() {
        this.code = ResponseCode.SUCCESS; // 응답 코드를 성공 코드로 초기화
        this.message = ResponseMessage.SUCCESS; // 응답 메시지를 성공 메시지로 초기화
    }

    public static ResponseEntity<ResponseDto> databaseError() { // 데이터베이스 오류 응답을 생성하는 메소드
        ResponseDto responseBody = new ResponseDto(ResponseCode.DATABASE_ERROR, ResponseMessage.DATABASE_ERROR); // 응답 객체 생성 및 응답 코드와 메시지 설정       
    ...
    public static ResponseEntity<ResponseDto> validationFail() { // 유효성 검사 실패 응답을 생성하는 메소드
        ResponseDto responseBody = new ResponseDto(ResponseCode.VALIDATION_FAIL, ResponseMessage.VALIDATION_FAIL); // 응답 객체 생성 및 응답 코드와 메시지 설정
